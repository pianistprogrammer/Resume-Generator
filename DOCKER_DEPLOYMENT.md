...
# Copy backup to host
docker cp jobalert-mongodb:/data/backup ./mongodb-backup-$(date +%Y%m%d)
```
...
1. **MongoDB Replica Set:**
   ...
   - Update `MONGODB_URL` in backend environment
...
- `MONGODB_URL` - Database connection (auto-set in Docker)
- `CELERY_BROKER_URL` - Redis URL (auto-set in Docker)
